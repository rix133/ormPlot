...
The main advantage of using ormPlot is that you get plots with coeficent intervals
shown on the plot.

The simplest way is to predict for only one value. Plotting returs a 
customisable ggplot object.
```{r}
...
You can easily set custom lables.
```{r}
...
```{r eval = FALSE}
ggsave("figures/educ_cran.svg",final_plot, height = 8 ,width = 8)
```
